...
TEST_CASE("Empty board") {
    ...
    board.setPiece(Square::H8, Piece(PieceColor::White, PieceType::King));

    std::cout << board << std::endl;
}

TEST_CASE("Test initialisation") {
    auto optBoard = Fen::createBoard("rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1");
    REQUIRE(optBoard.has_value());

    auto board = optBoard.value();
    REQUIRE(board.piece(Square::A1) == Piece(PieceColor::White, PieceType::Rook));
    REQUIRE(board.piece(Square::B1) == Piece(PieceColor::White, PieceType::Knight));
    REQUIRE(board.piece(Square::C1) == Piece(PieceColor::White, PieceType::Bishop));
    REQUIRE(board.piece(Square::D1) == Piece(PieceColor::White, PieceType::Queen));
    REQUIRE(board.piece(Square::E1) == Piece(PieceColor::White, PieceType::King));
    REQUIRE(board.piece(Square::F1) == Piece(PieceColor::White, PieceType::Bishop));
    REQUIRE(board.piece(Square::G1) == Piece(PieceColor::White, PieceType::Knight));
    REQUIRE(board.piece(Square::H1) == Piece(PieceColor::White, PieceType::Rook));
    REQUIRE(board.piece(Square::A2) == Piece(PieceColor::White, PieceType::Pawn));
    REQUIRE(board.piece(Square::B2) == Piece(PieceColor::White, PieceType::Pawn));
    REQUIRE(board.piece(Square::C2) == Piece(PieceColor::White, PieceType::Pawn));
    REQUIRE(board.piece(Square::D2) == Piece(PieceColor::White, PieceType::Pawn));
    REQUIRE(board.piece(Square::E2) == Piece(PieceColor::White, PieceType::Pawn));
    REQUIRE(board.piece(Square::F2) == Piece(PieceColor::White, PieceType::Pawn));
    REQUIRE(board.piece(Square::G2) == Piece(PieceColor::White, PieceType::Pawn));
    REQUIRE(board.piece(Square::H2) == Piece(PieceColor::White, PieceType::Pawn));
    REQUIRE(board.piece(Square::A7) == Piece(PieceColor::Black, PieceType::Pawn));
    REQUIRE(board.piece(Square::B7) == Piece(PieceColor::Black, PieceType::Pawn));
    REQUIRE(board.piece(Square::C7) == Piece(PieceColor::Black, PieceType::Pawn));
    REQUIRE(board.piece(Square::D7) == Piece(PieceColor::Black, PieceType::Pawn));
    REQUIRE(board.piece(Square::E7) == Piece(PieceColor::Black, PieceType::Pawn));
    REQUIRE(board.piece(Square::F7) == Piece(PieceColor::Black, PieceType::Pawn));
    REQUIRE(board.piece(Square::G7) == Piece(PieceColor::Black, PieceType::Pawn));
    REQUIRE(board.piece(Square::H7) == Piece(PieceColor::Black, PieceType::Pawn));
    REQUIRE(board.piece(Square::A8) == Piece(PieceColor::Black, PieceType::Rook));
    REQUIRE(board.piece(Square::B8) == Piece(PieceColor::Black, PieceType::Knight));
    REQUIRE(board.piece(Square::C8) == Piece(PieceColor::Black, PieceType::Bishop));
    REQUIRE(board.piece(Square::D8) == Piece(PieceColor::Black, PieceType::Queen));
    REQUIRE(board.piece(Square::E8) == Piece(PieceColor::Black, PieceType::King));
    REQUIRE(board.piece(Square::F8) == Piece(PieceColor::Black, PieceType::Bishop));
    REQUIRE(board.piece(Square::G8) == Piece(PieceColor::Black, PieceType::Knight));
    REQUIRE(board.piece(Square::H8) == Piece(PieceColor::Black, PieceType::Rook));

    std::cout << board << std::endl;
}

TEST_CASE("Test pawn moves from starting position") {
    auto optBoard = Fen::createBoard("rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1");
    REQUIRE(optBoard.has_value());

    auto board = optBoard.value();
    board.setTurn(PieceColor::White);
    auto empty = board.getEmptySquares();
    auto enemy = board.getEnemySquares(PieceColor::White);
    std::cout << MoveGenerator::pawnMoves(Square::A2, empty, enemy, board.turn(), board.enPassantSquare()) << std::endl;
}

TEST_CASE("Test queen moves") {
    auto board = Board();
    board.setTurn(PieceColor::White);
    board.setPiece(Square::B2, Piece(PieceColor::White, PieceType::Knight));
    auto empty = board.getEmptySquares();
    auto enemy = board.getEnemySquares(PieceColor::White);

    Board::printBitBoard(MoveGenerator::knightMoves(Square::B2, empty, enemy));
}

TEST_CASE("aaaaaaaaaaaaa") {
    auto board = Fen::createBoard("r3k2r/8/8/8/8/8/3PPP2/R3K2R w Kkq - 0 1").value();
    board.setTurn(PieceColor::White);
    auto empty = board.getEmptySquares();
    auto enemy = board.getEnemySquares(PieceColor::White);

    auto moves = MoveGenerator::knightMoves(Square::E5, empty, enemy);

    Board::printBitBoard(moves);

    Board::printBitBoard(board.getEnemyControlledSquares(PieceColor::White));
}

TEST_CASE("test cpp bullshit") {
    auto optBoard = Fen::createBoard("rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1");
    REQUIRE(optBoard.has_value());
    ...
    auto newBoard = board;
    newBoard.setPiece(Square::A1, Piece(PieceColor::White, PieceType::King));

    REQUIRE(board.piece(Square::A1) == Piece(PieceColor::White, PieceType::Rook));
}

TEST_CASE("bishop moving weird") {
    auto optBoard = Fen::createBoard("rnbqkbnr/pppp1ppp/8/4p3/4P3/5N2/PPPP1PPP/RNBQKB1R b KQkq - 1 2");
    REQUIRE(optBoard.has_value());

    auto board = optBoard.value();

    std::vector<Move> moves;
    board.pseudoLegalMovesFrom(Square::F8, moves);
...
TEST_CASE("test board state that crashed") {
    auto optBoard = Fen::createBoard("5bnr/5ppp/4k3/4p3/1QP1P3/3q1B2/3N1PPP/2B2RK1 b - - 0 24");
    REQUIRE(optBoard.has_value());
...
TEST_CASE("test checkmate") {

    auto optBoard = Fen::createBoard("5r2/1p4Qk/3p4/p2Pn3/4q3/1P6/P5RP/7K b - - 0 1");
    REQUIRE(optBoard.has_value());
    auto board = optBoard.value();

    REQUIRE(board.isCheckMate(board.turn()));
}
